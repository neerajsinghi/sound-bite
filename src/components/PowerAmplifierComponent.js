...
function RenderPowerAmplifier({ item }) {
    return (
        <Link to={`/powerAmplifier/${item.id}`} >
...
const PowerAmplifier = (props) => {
    const featuredItemsMap = props.powerAmplifiers.map((item) => {
        return (<div key={item.id} className="col-6 col-md-4 mb-3">
            <RenderPowerAmplifier item={item} />
        </div>);
    });
    return (
        <div className="products">
            <div className="row">
                <Breadcrumb >
                    ...
                    <BreadcrumbItem style={{ fontFamily: "Oswald,sans-serif !important" }} active>PowerAmplifiers</BreadcrumbItem>
                </Breadcrumb>
                <div className="col-12 m-3">
                    <h3 className='page-title'>PowerAmplifiers</h3>
