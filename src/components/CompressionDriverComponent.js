...
function RenderCompressionDriver({ item }) {
    return (
        <Link to={`/compressionDriver/${item.id}`} >
...
const CompressionDriver = (props) => {
    const featuredItemsMap = props.compressiondrivers.map((item) => {
        return (<div key={item.id} className="col-6 col-md-4 mb-3">
            <RenderCompressionDriver item={item} />
        </div>);
    });
    return (
        <div className="products">
            <div className="row">
                <Breadcrumb >
                    ...
                    <BreadcrumbItem style={{ fontFamily: "Oswald,sans-serif !important" }} active>CompressionDrivers</BreadcrumbItem>
                </Breadcrumb>
                <div className="col-12 m-3">
                    <h3 className='page-title'>CompressionDrivers</h3>
...
export default CompressionDriver;