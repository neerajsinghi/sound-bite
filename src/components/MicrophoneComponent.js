...
function RenderMicrophone({ item }) {
    return (
        <Link to={`/microphone/${item.id}`} >
...
const Microphone = (props) => {
    const featuredItemsMap = props.microphones.map((item) => {
        return (<div key={item.id} className="col-6 col-md-4 mb-3">
            <RenderMicrophone item={item} />
        </div>);
    });
    return (
        <div className="products">
            <div className="row">
                <Breadcrumb >
                    ...
                    <BreadcrumbItem style={{ fontFamily: "Oswald,sans-serif !important" }} active>Microphones</BreadcrumbItem>
                </Breadcrumb>
                <div className="col-12 m-3">
                    <h3 className='page-title'>Microphones</h3>
