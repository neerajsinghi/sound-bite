...
function RenderCrossoverNetwork({ item }) {
    return (
        <Link to={`/crossoverNetwork/${item.id}`} >
...
const CrossoverNetwork = (props) => {
    const featuredItemsMap = props.crossoverNetworks.map((item) => {
        return (<div key={item.id} className="col-6 col-md-4 mb-3">
            <RenderCrossoverNetwork item={item} />
        </div>);
    });
    return (
        <div className="products">
            <div className="row">
                <Breadcrumb >
                    ...
                    <BreadcrumbItem style={{ fontFamily: "Oswald,sans-serif !important" }} active>CrossoverNetworks</BreadcrumbItem>
                </Breadcrumb>
                <div className="col-12 m-3">
                    <h3 className='page-title'>CrossoverNetworks</h3>
...
export default CrossoverNetwork;