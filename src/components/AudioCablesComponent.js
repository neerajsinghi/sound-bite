...
function RenderAudioCable({ item }) {
    return (
        <Link to={`/audioCable/${item.id}`} >
...
const AudioCable = (props) => {
    const featuredItemsMap = props.audioCables.map((item) => {
        return (<div key={item.id} className="col-6 col-md-4 mb-3">
            <RenderAudioCable item={item} />
        </div>);
    });
    return (
        <div className="products">
            <div className="row">
                <Breadcrumb >
                    ...
                    <BreadcrumbItem style={{ fontFamily: "Oswald,sans-serif !important" }} active>AudioCables</BreadcrumbItem>
                </Breadcrumb>
                <div className="col-12 m-3">
                    <h3 className='page-title'>AudioCables</h3>
