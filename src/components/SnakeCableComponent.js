...
function RenderSnakeCable({ item }) {
    return (
        <Link to={`/snakeCable/${item.id}`} >
...
const SnakeCable = (props) => {
    const featuredItemsMap = props.snakeCables.map((item) => {
        return (<div key={item.id} className="col-6 col-md-4 mb-3">
            <RenderSnakeCable item={item} />
        </div>);
    });
    return (
        <div className="products">
            <div className="row">
                <Breadcrumb >
                    ...
                    <BreadcrumbItem style={{ fontFamily: "Oswald,sans-serif !important" }} active>SnakeCables</BreadcrumbItem>
                </Breadcrumb>
                <div className="col-12 m-3">
                    <h3 className='page-title'>SnakeCables</h3>
